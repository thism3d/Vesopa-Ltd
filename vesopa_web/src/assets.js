 ...
 *
 * A hand-maintained constant is the option that does not work, and this site
 * was the proof: `APP_VERSION = '1.3.2.0'` was pasted onto some assets as
 * `?v=<%= APP_VERSION %>` and onto others not at all, and nothing bumped it on
 * a deploy — so `main0.1.0.css?v=1.3.2.0` was one immortal URL and
 * `/script.js` was another. `APP_VERSION` stays for what it is actually for,
 * which is saying which version of the site this is; it is no longer asked to
 * be a cache key as well.
 *
 * This file is a copy of vesopa_server/src/assets.js. Two small self-contained
 * copies rather than a shared package, because the two apps deploy separately
 * and neither should be able to break the other's asset URLs.
 *
 * WHEN IT IS COMPUTED.
 *
...
const SKIP_DIRS = new Set([
  'uploads',
  'node_modules',
  // The downloadable installers. Hundreds of megabytes of .msix and .apk on
  // live, served with their own Content-Disposition, never referenced from a
  // stylesheet — hashing them would add minutes to every restart for nothing.
  // (The directory is nearly empty in a checkout, which is why this has to be
  // decided by what it is for and not by what is in it locally.)
  'app',
]);
