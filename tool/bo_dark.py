"""Photograph the Products page in dark mode, where the venue works."""
import pathlib
...
def main():
    ...
    with sync_playwright() as p:
        ...
        for path in ("/products", "/dashboard", "/mix-match", "/customers"):
            page.goto(f"{BASE}{path}", wait_until="networkidle")
            page.wait_for_timeout(1500)
            page.screenshot(
                path=str(OUT / f"before-dark-{path.strip('/')}.png")
            )
            print("shot", path)
        browser.close()
