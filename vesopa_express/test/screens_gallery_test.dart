...
// The meal's two questions, answered by products priced as the upgrade -- as
// the test venue has them on live (tool/express-demo-meals.js).
const _sides = AddOnGroup(id: 20, name: 'Choose your side', min: 1, max: 1, options: [
  AddOnOption(pluId: 169, name: 'Chips', priceMinor: 0),
  AddOnOption(pluId: 170, name: 'Sweet Potato Fries', priceMinor: 80),
  AddOnOption(pluId: 171, name: 'Onion Rings', priceMinor: 50, allergens: ['gluten']),
  AddOnOption(pluId: 172, name: 'Side Salad', priceMinor: 0),
]);
const _drinks = AddOnGroup(id: 21, name: 'Choose your drink', min: 1, max: 1, options: [
  AddOnOption(pluId: 173, name: 'Coca-Cola', priceMinor: 0),
  AddOnOption(pluId: 174, name: 'Diet Coke', priceMinor: 0),
  AddOnOption(pluId: 175, name: 'Orange Juice', priceMinor: 0),
  AddOnOption(pluId: 176, name: 'Still Water', priceMinor: 0),
  AddOnOption(pluId: 177, name: 'Ginger Beer', priceMinor: 40),
]);
...
const _burger = MenuItem(
  id: 1, pluId: 101, name: 'Kitchen Cheeseburger', priceMinor: 1150, popular: true,
  ...
  meals: [
    Meal(pluId: 178, name: 'Cheeseburger Meal', priceMinor: 1450, label: 'Regular', steps: [_sides, _drinks]),
    Meal(pluId: 179, name: 'Cheeseburger Large Meal', priceMinor: 1550, label: 'Large', steps: [_sides, _drinks]),
  ],
);
...
// A two-line name, a diet tag AND a meal on one card: the combination that
// pushed the price out of a 208-pixel landscape card on the first live run.
const _vegan = MenuItem(id: 3, pluId: 103, name: 'Beetroot & Bean Burger', priceMinor: 995, diet: 'vegan',
    allergensDeclared: true,
    meals: [Meal(pluId: 181, name: 'Beetroot & Bean Burger Meal', priceMinor: 1295, steps: [_sides, _drinks])]);
...
void main() {
  ...
  testWidgets('portrait_04_item_sheet', (tester) async {
    ...
    await tester.tap(find.descendant(of: find.byType(ItemCard), matching: find.text('Kitchen Cheeseburger')).first);
    for (var i = 0; i < 12; i++) {
      await tester.pump(const Duration(milliseconds: 100));
    }
  ...
  // Make it a meal: the choice on the dish, then size, side, drink and the
  // meal as it will arrive -- photographed at each step, in each shape, and
  // ending in exactly the basket line the customer chose.
  for (final (shape, size) in const [
    ('portrait', Size(1080, 1920)),
    ('landscape', Size(1920, 1080)),
    ('small', Size(540, 960)),
  ]) {
    testWidgets('${shape}_25_meal', (tester) async {
      SharedPreferences.setMockInitialValues({});
      tester.view.physicalSize = size;
      tester.view.devicePixelRatio = 1;
      addTearDown(tester.view.reset);
      ...
      await tester.pump();
      final container = ProviderScope.containerOf(tester.element(find.byType(ExpressApp)));
      container.read(orderFlowProvider.notifier).state = const FlowState(step: FlowStep.menu, orderType: 'take_away');
      ...
      await settle(3);
      await expectLater(find.byType(ExpressApp), matchesGoldenFile('gallery/${shape}_25_meal_choice.png'));

      await tester.tap(find.text('Make it a meal').last);
      await settle(4);
      await expectLater(find.byType(ExpressApp), matchesGoldenFile('gallery/${shape}_26_meal_size.png'));

      await tester.tap(find.text('Large').last);
      await settle(6);
      expect(find.text('Choose your side'), findsWidgets, reason: 'choosing a size moves on by itself');
      // Scrolled to, as a customer would: on a small screen the answers run
      // below the fold of the step.
      await tester.ensureVisible(find.text('Sweet Potato Fries'));
      await tester.tap(find.text('Sweet Potato Fries'));
      await settle(2);
      await expectLater(find.byType(ExpressApp), matchesGoldenFile('gallery/${shape}_27_meal_side.png'));
      await settle(6);

      expect(find.text('Choose your drink'), findsWidgets);
      await tester.ensureVisible(find.text('Ginger Beer'));
      await settle(2);
      await tester.tap(find.text('Ginger Beer'));
      await settle(8);
      await expectLater(find.byType(ExpressApp), matchesGoldenFile('gallery/${shape}_28_meal_review.png'));

      await tester.tap(find.byKey(const ValueKey('add-meal')));
      await settle(12);
      final line = container.read(orderFlowProvider).basket.lines.single;
      expect(line.meal?.name, 'Cheeseburger Large Meal');
      expect(line.addOns.map((a) => a.name), ['Sweet Potato Fries', 'Ginger Beer']);
      // 15.50 + 0.80 + 0.40, the figures the server will charge.
      expect(line.totalMinor, 1670);
      expect(line.toRequest(), {'item_id': 1, 'qty': 1, 'add_ons': [170, 177], 'meal_plu': 179});
    });
  }

  // The number, at a venue that offers a ticket, on a kiosk with a printer.
  testWidgets('portrait_29_done_receipt', (tester) async {
    SharedPreferences.setMockInitialValues({
      'express_ticket_printer': '{"kind":"network","host":"192.168.1.50","port":9100}',
    });
    tester.view.physicalSize = const Size(1080, 1920);
    tester.view.devicePixelRatio = 1;
    addTearDown(tester.view.reset);
    ...
    await tester.pump();
    final container = ProviderScope.containerOf(tester.element(find.byType(ExpressApp)));
    container.read(orderFlowProvider.notifier).state = const FlowState(
      step: FlowStep.done,
      orderType: 'take_away',
      order: OrderView(publicId: 'p', number: 42, status: 'paid', stage: PayStage.paid, totalMinor: 1670,
          orderType: 'take_away', customerName: 'Rhys'),
    );
    for (var i = 0; i < 8; i++) {
      await tester.pump(const Duration(milliseconds: 100));
    }
    expect(find.text('Print a receipt'), findsOneWidget);
    await expectLater(find.byType(ExpressApp), matchesGoldenFile('gallery/portrait_29_done_receipt.png'));
  });
}
