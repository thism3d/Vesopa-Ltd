...
/// What the kiosk prints at the top and bottom of a ticket, and whether it
/// prints one at all: the venue's own receipt branding, from the back office,
/// so a customer's paper from the kiosk and from the counter agree.
@immutable
class ReceiptFace {
  const ReceiptFace({
    this.mode = 'ask',
  ...
  /// always | ask | never. See schema_till_express.sql.
  final String mode;
  ...
  bool get always => mode == 'always';
  bool get ask => mode == 'ask';
  bool get never => mode == 'never';

  factory ReceiptFace.fromJson(Object? raw) {
    final j = _map(raw);
    final mode = _str(j['mode']);
    return ReceiptFace(
      mode: const {'always', 'ask', 'never'}.contains(mode) ? mode! : 'ask',
      venueName: _str(j['venue_name']),
      address: [for (final a in _list(j['address'])) ?_str(a)],
      phone: _str(j['phone']),
      vatNumber: _str(j['vat_number']),
      companyNumber: _str(j['company_number']),
      footer: _str(j['footer']),
      footerNote: _str(j['footer_note']),
    );
  }
}
...
class KioskConfig {
  ...
  /// The languages to offer, by code, the first the default. The server
  /// decides: Welsh is written but hidden until a Welsh speaker has checked it,
...
/// this was sent offers English only -- which is also what a kiosk should do
/// with an answer it cannot read.
List<String> _languages(Object? raw) {
  final known = [
    for (final l in _list(raw))
      if (const {'en', 'cy'}.contains('$l')) '$l',
  ];
  return known.isEmpty ? const ['en'] : known;
}
...
class AddOnOption {
  ...
  /// A picture, when the venue has one for this answer -- sent for a meal's
  /// answers, where the step is a grid of cards rather than a row of chips.
...
/// "Make it a meal": one size of the meal a dish offers.
///
/// A product of its own in the catalogue -- "Cheeseburger Meal" at the meal
/// price -- whose questions are the steps the kiosk walks through. The kiosk
/// only ever shows the price; the server charges what the catalogue says.
@immutable
class Meal {
  const Meal({
    required this.pluId,
  ...
  /// "Regular", "Large" -- null when the dish offers one meal.
  final String? label;
  final String? imageUrl;
  final List<String> allergens;
  final List<AddOnGroup> steps;
  ...
  factory Meal.fromJson(Map<String, dynamic> j) => Meal(
    ...
    allergens: [for (final a in _list(j['allergens'])) '$a'],
    steps: [
      for (final g in _list(j['steps'])) AddOnGroup.fromJson(_map(g)),
...
class MenuItem {
  ...
  /// The meals this dish offers, in the venue's order. Empty for most dishes.
  final List<Meal> meals;

  /// The cheapest way to have it as a meal: what "from £16.50" says.
  int? get mealFromMinor =>
      meals.isEmpty ? null : meals.map((m) => m.priceMinor).reduce((a, b) => a < b ? a : b);
  ...
  factory MenuItem.fromJson(Map<String, dynamic> j) => MenuItem(
    ...
    meals: [for (final m in _list(j['meals'])) Meal.fromJson(_map(m))],
  );
}
