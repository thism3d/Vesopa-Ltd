...
@immutable
class BasketLine {
  const BasketLine({required this.item, this.addOns = const [], this.qty = 1, this.meal});

  final MenuItem item;
  final List<AddOnOption> addOns;
  final int qty;

  /// The meal this dish was made into, or null for the dish on its own. Its
  /// answers are in [addOns], in the order the steps asked them.
  final Meal? meal;

  /// What the customer reads for this line: "Cheeseburger Meal", or the dish.
  String get name => meal?.name ?? item.name;

  /// Two burgers with the same extras are one line of two; with different
  /// extras -- or one as a meal -- they are two lines. The server merges on
  /// exactly this key, and a basket that sent the same key twice would have the
  /// second overwrite the first -- so it never does.
  String get key {
    final plus = addOns.map((a) => a.pluId).toList()..sort();
    return '${item.id}|${meal?.pluId ?? ''}|${plus.join('+')}';
  }

  int get unitMinor =>
      (meal?.priceMinor ?? item.priceMinor) + addOns.fold(0, (s, a) => s + a.priceMinor);

  int get totalMinor => unitMinor * qty;

  BasketLine withQty(int qty) => BasketLine(item: item, addOns: addOns, qty: qty, meal: meal);
