...
/// on the UI isolate — see `TicketPrinter`, which runs them on a worker.
///
/// A COPY of vesopa_epos/lib/printing/windows_printing.dart, taken whole
/// because it is the part of the till's printing that has been proven on real
/// counters -- the SetupAPI sizing trap, the RAW datatype, the short writes. A
/// fix to one belongs in both.
library;
...
void _requireWindows() {
  if (!_isWindows) {
    throw WindowsPrintException(
      'Direct Windows printing is only available on a Windows kiosk.',
    );
  }
}
