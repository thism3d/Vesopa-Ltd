    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          // Only when there is another language to offer: see strings.dart for
          // why Welsh is hidden for now.
  ...
  void _accessibility(BuildContext context, WidgetRef ref) {
    showModalBottomSheet<void>(
      ...
      builder: (context) => Consumer(builder: (context, ref, _) {
        ...
        final s = flow.s;
        final multilingual = ref.watch(kioskSessionProvider.select((st) => st.config?.multilingual ?? false));
        return SafeArea(
          child: Padding(
            ...
            AspectRatio(
              ...
              child: Stack(
                ...
                children: [
                  ...
                  // A dish that can be a meal says so on its card, as it does
                  // at the counter: nobody opens a dish to find out. On the
                  // picture rather than under the name -- under it, beside a
                  // two-line name, it pushed the price out of a landscape card
                  // by 4 pixels on the first live run.
                  if (item.meals.isNotEmpty)
                    Positioned(
                      left: 10,
                      bottom: 10,
                      child: Pill(s('make_it_a_meal'), color: Colors.white, textColor: Xp.ink,
    ...
    ..showSnackBar(
      SnackBar(
        ...
        content: Row(children: [
          ...
          Expanded(
            child: Text('${added.qty} x ${added.name}',
...
class _ItemSheetState extends ConsumerState<ItemSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_meal) {
      return MealBuilder(
        item: item,
        onBack: () => setState(() => _meal = false),
        onDone: (line) => Navigator.of(context).pop(line),
      );
    }
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (item.meals.isNotEmpty) ...[
                          const SizedBox(height: 22),
                          MealChoice(item: item, s: s, onMeal: () => setState(() => _meal = true)),
                        ],
...
class _BasketLineTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final thumb = ClipRRect(
      borderRadius: BorderRadius.circular(16),
      child: SizedBox(width: 96, height: 96, child: NetImage(line.meal?.imageUrl ?? line.item.imageUrl)),
    );
    final words = Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      mainAxisSize: MainAxisSize.min,
      children: [
        Text(line.name, style: text.titleLarge),
