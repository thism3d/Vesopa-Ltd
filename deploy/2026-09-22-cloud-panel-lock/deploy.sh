#!/usr/bin/env bash
# Upload and install the cloud.vesopa.com panel, lock and AI switch.
set -euo pipefail
...
echo "mkdir -p /root/deploy_cloud_panel && chmod 700 /root/deploy_cloud_panel" > "$HERE/.mk"
...
"$PUTTY/pscp.exe" -batch -hostkey "$HOSTKEY" -pw "$PW" -q \
  "$(cygpath -w "$HERE/remote-install.sh")" "$(cygpath -w "$HERE/bundle.tgz")" root@34.63.118.67:/root/deploy_cloud_panel/
echo "bash /root/deploy_cloud_panel/remote-install.sh" > "$HERE/.run"
