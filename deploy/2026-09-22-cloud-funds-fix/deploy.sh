#!/usr/bin/env bash
# Upload and install the cloud.vesopa.com insufficient-balance fix.
set -euo pipefail
...
echo "mkdir -p /root/deploy_cloud_funds && chmod 700 /root/deploy_cloud_funds" > "$HERE/.mk"
...
"$PUTTY/pscp.exe" -batch -hostkey "$HOSTKEY" -pw "$PW" -q \
  "$(cygpath -w "$HERE/remote-install.sh")" "$(cygpath -w "$HERE/cloud-fix.tgz")" root@34.63.118.67:/root/deploy_cloud_funds/
echo "bash /root/deploy_cloud_funds/remote-install.sh" > "$HERE/.run"
