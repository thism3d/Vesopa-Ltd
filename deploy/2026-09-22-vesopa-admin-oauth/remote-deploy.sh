...
authz=$(curl -s -o /dev/null -w '%{http_code}' --max-time 15 "$loc")
[ "$authz" = 200 ] && ok "auth accepts the admin client's authorize request (200)" || { echo "  authorize answered $authz"; fail=1; }
[ "$(curl -s -o /dev/null -w '%{http_code}' -X POST --max-time 15 https://auth.vesopa.com/api/app/venues/provision)" = 401 ] \
