...
#
# VESOPA'S OWN COPY. It began as HestiaCP's hst-install-ubuntu.sh and is
# maintained here from now on; nothing at install time is fetched from their
# GitHub. Run THIS FILE directly.
#
# Do not reintroduce their hst-install.sh wrapper. Its only job is
#
#     wget -q https://.../hst-install-ubuntu.sh -O hst-install-ubuntu.sh
#
# which overwrites whatever is on disk, every run, with no prompt — so running
# it would silently replace this file with the unfixed upstream one and undo
# every fix below before the install even starts.
#
# Every local change is marked "VESOPA:" in place. The frozen copy it was forked
# from sits beside this file, so
#
#     diff -u upstream-reference/hst-install-ubuntu.sh vesopa-hestia-install.sh
#
# always shows exactly what we changed. Keep that: it is the only thing that
# makes a later rebase onto a newer HestiaCP tractable.
#
# WHAT IS STILL NOT OURS
#
# Owning this script does not own the software it installs. The panel itself is
# .deb packages from $RHOST (apt.hestiacp.com below, overridable with
# HESTIA_APT_HOST). To own the base properly, mirror that apt repository or
# build the debs and pass --with-debs /path. Until then the script is ours and
# the packages are still theirs.
#
# ============================================================================ #
...
# VESOPA: overridable, so pointing a build at our own package mirror does not
# mean editing this file. Default unchanged.
RHOST="${HESTIA_APT_HOST:-apt.hestiacp.com}"
VERSION='ubuntu'
...
if [ -d /etc/netplan ] && [ -z "$force" ]; then
	if [ -z "$(ls -A /etc/netplan)" ]; then
		...
		# VESOPA: answered automatically. This prompt is covered by NEITHER
		# --interactive nor --force, so it hangs an unattended run on a box
		# configured through systemd-networkd or a cloud guest agent — which is
		# most GCE and EC2 images. Its default is already "continue" (an empty
		# answer is accepted below), so this changes no outcome; it only stops
		# the installer waiting for a keypress nobody is there to give.
		answer=Y
...
# VESOPA: was a hard abort, and it is the single thing that would have made
# owning this file pointless.
#
# Upstream fetched src/deb/hestia/control from their release branch AT INSTALL
# TIME and refused to run unless this script's version string matched whatever
# was on that branch at that moment. Two consequences, both bad for us:
#
#   1. The day HestiaCP bump their release branch, this pinned copy stops
#      installing — with a message telling you to go and re-download their
#      script, which would throw away every fix in this file.
#   2. If raw.githubusercontent.com is unreachable or rate-limited, the lookup
#      returns an empty string, the comparison fails, and the install aborts on
#      a network blip that has nothing to do with the server being built.
#
# The check is worth KEEPING as information: this script and the .deb packages
# it installs do have to be compatible, and a mismatch is worth knowing about.
# It is not worth aborting on, now that divergence is deliberate. So it warns,
# says what to do about it, and continues. Set HESTIA_STRICT_VERSION=yes to get
# the old abort back.
#
# If a mismatch ever does produce a broken panel, the fix is to stop floating on
# their apt repo: build the debs and pass --with-debs, or point HESTIA_APT_HOST
# at our own mirror.
if [ -z "$withdebs" ] || [ ! -d "$withdebs" ]; then
	release_branch_ver=$(curl -s --max-time 10 https://raw.githubusercontent.com/hestiacp/hestiacp/release/src/deb/hestia/control 2> /dev/null | grep "Version:" | awk '{print $2}')
	if [ -z "$release_branch_ver" ]; then
		echo -e "\e[33m[ ! ] Could not read the upstream package version (offline, or GitHub unreachable).\e[0m"
		echo -e "\e[33m      Continuing — this check is advisory.\e[0m"
	elif [ "$HESTIA_INSTALL_VER" != "$release_branch_ver" ]; then
		echo
		echo -e "\e[33m[ ! ] Version notice\e[0m"
		echo "===================================================================="
		echo -e "\e[33m      This installer is $HESTIA_INSTALL_VER; the packages in the apt repo are $release_branch_ver.\e[0m"
		echo -e "\e[33m      That is expected while we maintain our own copy of this script.\e[0m"
		echo -e "\e[33m      If the panel misbehaves afterwards, pin the packages instead of floating:\e[0m"
		echo -e "        \e[33m./hst_autocompile.sh --hestia branchname no\e[0m"
		echo -e "        \e[33mbash vesopa-hestia-install.sh .. --with-debs /tmp/hestiacp-src/debs\e[0m"
		echo "===================================================================="
		echo
		if [ "${HESTIA_STRICT_VERSION:-no}" = "yes" ]; then
			check_result 1 "Installation aborted (HESTIA_STRICT_VERSION=yes)"
		fi
	fi
fi
...
mkdir -p $hst_backups
# VESOPA: the cd is checked and the mkdirs take -p. Unguarded, a failed cd
# scattered directories called "nginx", "mysql" and so on into whatever
# directory the installer happened to be launched from; without -p the second
# run of the installer died here on "File exists".
cd "$hst_backups" || { echo "Error: cannot enter $hst_backups"; exit 1; }
mkdir -p nginx apache2 php vsftpd proftpd bind exim4 dovecot clamd
mkdir -p spamassassin mysql postgresql openssl hestia
...
# VESOPA: guarded. A second run died with "useradd: user 'hestiaweb' already
# exists". The hestia-users group created just below IS guarded upstream, which
# is how you can tell this was an oversight rather than a decision.
id -u hestiaweb > /dev/null 2>&1 || /usr/sbin/useradd "hestiaweb" -c "$email" --no-create-home
...
# VESOPA: guarded, same reason as hestiaweb above.
id -u hestiamail > /dev/null 2>&1 || /usr/sbin/useradd "hestiamail" -c "$email" --no-create-home
...
# Configuring NTP
#
# VESOPA: rewritten. Upstream treated "the release is 22.04 or 24.04" as proof
# that systemd-timesyncd is installed. It is not. In 24.04 timesyncd is a
# SEPARATE PACKAGE, split out of systemd, and it is absent from the cloud and
# minimal images that Google Cloud and AWS actually boot — verified on a running
# 24.04 host: no package, no unit file, no /etc/systemd/timesyncd.conf. So both
# lines failed:
#
...
#
# Upstream's own comment on the next line admits the package is not guaranteed
# ("26.04 ... uses Chrony"); the check was simply written for the wrong release.
#
# The rule here is INSTALL THE MISSING PACKAGE, never skip and never abort. A
# control panel needs a disciplined clock — DKIM signatures, TOTP logins and
# Let's Encrypt all depend on it — but a clock that cannot be disciplined is a
# warning, not a reason to refuse to build the server.
vesopa_configure_ntp() {
	# Already synchronised, by whatever means: leave it alone.
	if timedatectl show -p NTPSynchronized --value 2> /dev/null | grep -q yes; then
		echo "[ * ] Clock is already synchronised"
		return 0
	fi

	# Something else may already own time on this box — GCE images often ship
	# chrony — and two NTP daemons fighting over the clock is worse than one.
	local unit
	for unit in chrony chronyd ntpsec ntp systemd-timesyncd; do
		if systemctl is-active --quiet "$unit" 2> /dev/null; then
			echo "[ * ] Time sync already handled by $unit"
			return 0
		fi
	done

	echo "[ * ] No time client running, installing one..."
	if ! systemctl list-unit-files systemd-timesyncd.service > /dev/null 2>&1; then
		DEBIAN_FRONTEND=noninteractive apt-get -y install systemd-timesyncd >> $LOG 2>&1 || true
	fi

	if systemctl list-unit-files systemd-timesyncd.service > /dev/null 2>&1; then
		# Only touch the config once we know the package put it there.
	...
	if DEBIAN_FRONTEND=noninteractive apt-get -y install chrony >> $LOG 2>&1; then
		systemctl enable chrony > /dev/null 2>&1 || true
		systemctl start chrony > /dev/null 2>&1 || true
		echo "[ * ] chrony enabled (systemd-timesyncd unavailable)"
		return 0
	fi

	echo "[ ! ] Could not install any NTP client. Set the clock before issuing certificates."
	return 0
}
vesopa_configure_ntp
...
# VESOPA: -f so a re-run does not die with "File exists", -n so it replaces
# the link rather than creating one inside the directory it points at.
ln -sfn /var/log/hestia $HESTIA/log
...
# VESOPA: everything below writes with RELATIVE paths, so a failed cd put the
# panel's certificate and its PRIVATE KEY in the current directory instead.
mkdir -p "$HESTIA/ssl"
cd "$HESTIA/ssl" || { echo "Error: cannot enter $HESTIA/ssl"; exit 1; }
...
if [ "$mysql" = 'yes' ] || [ "$mysql8" = 'yes' ]; then
	...
	mkdir -p /usr/share/phpmyadmin/tmp  # VESOPA: -p, dies on a re-run otherwise
...
if [ "$postgresql" = 'yes' ]; then
	...
	ln -sf /etc/phppgadmin/config.inc.php /usr/share/phppgadmin/conf/  # VESOPA: -f, re-run safe
...
' /etc/exim4/exim4.conf.template
	...
	ln -sf /usr/sbin/exim4 /etc/alternatives/mta  # VESOPA: -f, re-run safe
...
if [ "$sieve" = 'yes' ]; then
	...
	if [ -d "/var/lib/roundcube" ]; then
		...
		ln -sf $RC_CONFIG_DIR/plugins/managesieve/config.inc.php $RC_INSTALL_DIR/plugins/managesieve/config.inc.php  # VESOPA: -f, re-run safe
