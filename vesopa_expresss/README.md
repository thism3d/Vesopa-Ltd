...
come later from the same code. **Not yet on the Microsoft Store** (see
*Packaging*).
...
`msix_config` in `pubspec.yaml` is prepared with **placeholder** identity values
and `store: false`. When the owner decides to publish, Partner Center reserves
the name and assigns `identity_name` and `publisher`; copy both in exactly (as
the till and the kitchen do), set `store: true`, and submit through
`ms-store-submission-client/`. Until then `dart run msix:create` makes a
locally signed package for testing on a kiosk only.
