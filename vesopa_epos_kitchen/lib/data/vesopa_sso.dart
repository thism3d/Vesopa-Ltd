...
/// Commissioning a kitchen screen with a Vesopa account.
///
/// A copy of `vesopa_epos/lib/data/vesopa_sso.dart`, and deliberately a
/// copy rather than a shared package: the two apps ship separately, are
/// certified separately by the Store, and a shared package would mean a
/// change to the till's sign-in could break a kitchen screen in a venue
/// that never installed the new till. Keep them in step by hand; the file
/// is eighty lines and changes about once a year.
///
...
class VesopaSso {
  ...
  /// The `vesopa-epos` client id, which the kitchen shares.
  ///
  /// The same public client as the till, on purpose. Both are Vesopa's own
  /// native applications on a venue's own machines, both hold no secret, and
  /// the token either produces is only worth anything to Vesopa's own back
  /// office -- which decides what the person may commission from their
  /// back-office account, not from which application asked. A second client id
  /// would look like separation and enforce none.
  /// Public — it identifies, it does not
