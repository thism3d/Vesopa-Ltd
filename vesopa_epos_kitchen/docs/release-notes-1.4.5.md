# Vesopa Kitchen 1.4.5 — release notes

Store submission: `msix_version: 1.4.5.0` (previous submission was 1.4.0.0).
Flutter `version: 1.4.5+6`.

The fourth part of a Store version must be 0 — Microsoft reserves the revision
field — so the numbering walks the third part. The comment beside
`msix_version` in pubspec.yaml has said so since this app was packaged.

Jumped from 1.4.0 to 1.4.5 to sit level with Vesopa EPOS, which is what this
...
**Update this alongside the till.** A till on 1.4.5 sends modifier lines to the
...
```
Version 1.4.5.0 – Modifiers On The Board

Works with Vesopa EPOS 1.4.5.
...
3. `msix_version` can never be reused, and its fourth part must stay 0 — the
